...
# إعداد بسيط ومباشر للمجلد الرئيسي
app = Flask(__name__)

# استخدام مجلد tmp السريع والمسموح به في Railway
AUDIO_DIR = "/tmp"
...
def process():
    ...
    try:
        ...
        sys_msg = "Role: Visual Assistant. Arabic language. No markdown."
        if mode == 'read': sys_msg = "Role: OCR. Arabic text only."
        ...
        resp = requests.post(
            ...
            json={"model": "google/gemini-2.0-flash-001", "messages": chat_history},
            timeout=30
        )
        ...
        return jsonify({'audio_url': f'/get_audio?fn={fname}'})
